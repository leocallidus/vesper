...
use gtk4::prelude::*;
use gtk4::{Application, Box as GtkBox, Orientation, Align};
...
use zbus::blocking::{Connection as DbusConnection, ConnectionBuilder as DbusConnectionBuilder, Proxy as DbusProxy};
use zbus::zvariant::OwnedObjectPath;
use zbus::interface;

use config::{ActivationLogEntry, AnimatedPattern, Config, ScreensaverMode, SettingsProfile, ACTION_START, ACTION_STOP};
use i18n::{Language, resolve_language, tr, yes_no, system_language, cli_usage};
use ui::{SettingsWindow, ScreensaverWindow};
...
const APP_ID: &str = "com.example.rs-screensaver";
const KDE_COMPONENT_FRIENDLY: &str = "RS Screensaver";
...
struct AppState {
    ...
    screensaver_windows: Arc<Mutex<Option<ScreensaverWindow>>>,
        ...
        loop {
            ...
            if screensaver_active {
                ...
                if is_enabled && is_idle {
                    let threshold_secs = monitor_config.lock().unwrap().active_profile().inactivity_seconds;
...
fn handle_cli_command() -> Option<i32> {
    ...
    let result = match command.as_str() {
        ...
        "disable" => run_cli_with_service(lang, || send_dbus_command("set_enabled", (false,), lang)),
        "inhibit" => {
            run_cli_with_service(lang, || send_dbus_command("set_inhibit_sleep", (true,), lang))
        }
        "uninhibit" => {
            run_cli_with_service(lang, || send_dbus_command("set_inhibit_sleep", (false,), lang))
        }
        "set-enabled" => {
            let value = args.pop().unwrap_or_default();
            match parse_bool_arg(&value, lang) {
                Ok(enabled) => {
                    run_cli_with_service(lang, || send_dbus_command("set_enabled", (enabled,), lang))
                }
                Err(err) => Err(err),
            }
        }
        "set-inhibit" => {
            let value = args.pop().unwrap_or_default();
            match parse_bool_arg(&value, lang) {
                Ok(inhibit) => run_cli_with_service(
                    lang,
    ...
    T: zbus::zvariant::Type + serde::ser::Serialize,
{
    ...
    let proxy = DbusProxy::new(&conn, DBUS_SERVICE_NAME, DBUS_OBJECT_PATH, DBUS_INTERFACE_NAME)
...
fn query_status(lang: Language) -> Result<Option<StatusSnapshot>, String> {
    ...
    let proxy = DbusProxy::new(&conn, DBUS_SERVICE_NAME, DBUS_OBJECT_PATH, DBUS_INTERFACE_NAME)
        .map_err(|err| err.to_string())?;
    let (active, enabled, inhibit, profile_index, profile_name, mode) = call_dbus_method::<
...
fn format_status_line(status: &StatusSnapshot, lang: Language) -> String {
    ...
    tr(lang, "Активен: {active} • Включен: {enabled} • Сон: {inhibit} • Профиль: {} ({}) • Режим: {}")
...
fn parse_bool_arg(value: &str, lang: Language) -> Result<bool, String> {
    match value.to_lowercase().as_str() {
        ...
        _ => Err(tr(lang, "Неизвестное значение: {value} (ожидается true/false).")
            .replace("{value}", value)),
    }
}
...
fn print_cli_error(message: &str) -> i32 {
    let lang = cli_language();
    eprintln!("{}", tr(lang, "Ошибка: {message}").replace("{message}", message));
    1
}
...
fn register_kde_global_hotkeys(
    ...
    let mut signals = component_proxy.receive_signal_with_args(
...
fn stop_screensaver(state: Arc<AppState>, reason: StopReason) {
    ...
    *active = false;
    ...
    if let Some(screensaver) = state.screensaver_windows.lock().unwrap().take() {
        screensaver.hide();
    }
...
fn start_screensaver(state: Arc<AppState>) {
    ...
    if let Err(message) = validate_profile_for_start(&profile, lang) {
        notify_problem(&state, &message, lang);
        return;
    }
    ...
    let app = state.main_window.lock().unwrap()
        .as_ref()
    ...
    let screensaver = ScreensaverWindow::new(&profile, app, started_at.clone(), move || {
        glib::idle_add_local_once({
            let state = Arc::clone(&state_clone);
            move || stop_screensaver(state, StopReason::UserActivity)
        });
    });
    
    *state.screensaver_active.lock().unwrap() = true;
    *state.screensaver_started_at.lock().unwrap() = Some(Instant::now());
    *state.screensaver_windows.lock().unwrap() = Some(screensaver);
    
    if let Some(ref screensaver) = *state.screensaver_windows.lock().unwrap() {
        screensaver.show();
    }
...
fn format_activation_mode(profile: &SettingsProfile, lang: Language) -> String {
    let (label, detail) = match &profile.mode {
        ...
        ScreensaverMode::Gradient { start, end } => (tr(lang, "Градиент"), Some(format!("{start} -> {end}"))),
        ScreensaverMode::Pattern(pattern) => (tr(lang, "Паттерн"), Some(pattern_label(*pattern, lang).to_string())),
    ...
    let detail = if profile.random_media
        && matches!(&profile.mode, ScreensaverMode::Image(_) | ScreensaverMode::Video(_))
...
fn request_mpris_pause() {
    thread::spawn(|| {
        if let Err(err) = pause_mpris_players() {
            eprintln!("MPRIS pause failed: {}", err);
        }
    });
}

fn pause_mpris_players() -> Result<(), Box<dyn std::error::Error>> {
    let conn = DbusConnection::session()?;
...
fn notify_problem(state: &AppState, message: &str, lang: Language) {
    ...
    if let Some(app) = state.main_window.lock().unwrap().as_ref().and_then(|w| w.application()) {
        let notification = Notification::new("RS Screensaver");
        notification.set_body(Some(&body));
        app.send_notification(Some("rs-screensaver-problem"), &notification);
    } else {
        eprintln!("{body}");
    }
}
...
fn validate_profile_for_start(profile: &SettingsProfile, lang: Language) -> Result<(), String> {
    match &profile.mode {
        ScreensaverMode::Image(path) => {
            validate_media_selection(tr(lang, "Изображение"), MediaKind::Image, profile, path, lang)
        }
...
fn validate_media_selection(
    ...
    if profile.random_media {
        ...
        if !path.trim().is_empty() {
            return validate_media_path(path, kind, lang)
    ...
    *state.inhibit_sleep.lock().unwrap() = inhibit;
    ...
    if inhibit {
        ...
        match std::process::Command::new("systemd-inhibit")
            .args(["--what=sleep:idle", "--who=RS Screensaver", "--why=User requested", "--mode=block", "sleep", "infinity"])
            .spawn()
        {
            Ok(child) => {
